...
// add needs c.Lock() before calling.
func (c *Consistent) add(elt string) {
...
// remove needs c.Lock() before calling.
func (c *Consistent) remove(elt string) {
	for i := 0; i < c.NumberOfReplicas; i++ {
...
// If there are existing elements not present in elts, they will be removed.
...
// GetTwo returns two closest distinct elements to the name input in the circle.
...
func (c *Consistent) GetN(name string, n int) ([]string, error) {
	...
	for i = start + 1; i != start; i++ {
		...
		elem = c.circle[c.sortedHashes[i]]
		if !sliceContainMember(res, elem) {
			res = append(res, elem)
		}
		if len(res) == n {
			break
...
func sliceContainMember(set []string, member string) bool {
	for _, m := range set {
		if m == member {
			return true
		}
